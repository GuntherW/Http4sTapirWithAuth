...
object HelloRoutes {
  ...
  private[routes] def helloGet1Routes(implicit config: ServiceConfig): HttpRoutes[Task] = Http4sServerInterpreter[Task]().toRoutes(HelloEndpoints.helloGet1) {
    case (authUser, n) =>
      HelloService.hellosSimple(n, authUser)
  }

  private[routes] def helloGet2Routes(implicit config: ServiceConfig): HttpRoutes[Task] = Http4sServerInterpreter[Task]().toRoutes(HelloEndpoints.helloGet2) {
    case (authUser, n) =>
      HelloService.helloReader(n, authUser).run(config)
  }

  private[routes] def helloGet3Routes(implicit config: ServiceConfig): HttpRoutes[Task] = Http4sServerInterpreter[Task]().toRoutes(HelloEndpoints.helloGet3) {
    case (authUser, n) =>
  ...
  private[routes] def helloPost1Routes(implicit config: ServiceConfig): HttpRoutes[Task] = Http4sServerInterpreter[Task]().toRoutes(HelloEndpoints.helloPost1) {
